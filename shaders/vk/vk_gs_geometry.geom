...
layout(location = 1) out vec3 fPosInfo;
...
void generate_circle() {
  ...
  fPosInfo = vec3(1.0, 0.0, 0.0);
  for (int i = 0; i < MAX_QUAD_STEP - 1; i++) {
    gl_Position = GlobalInfo.mvp * TransformData.uMatrix * gl_in[0].gl_Position;
    EmitVertex();

    gl_Position =
        GlobalInfo.mvp * TransformData.uMatrix * vec4(points[i], 0.0, 1.0);
    EmitVertex();

    gl_Position =
        GlobalInfo.mvp * TransformData.uMatrix * vec4(points[i + 1], 0.0, 1.0);
    EmitVertex();
...
void generate_normal_triangle() {
  gl_Position =
      GlobalInfo.mvp * TransformData.uMatrix * vec4(vPos[0].xy, 0.0, 1.0);
  fPos = vPos[0];
  fPosInfo = vPosInfo[0];
  EmitVertex();

  gl_Position =
      GlobalInfo.mvp * TransformData.uMatrix * vec4(vPos[1].xy, 0.0, 1.0);
  fPos = vPos[1];
  fPosInfo = vPosInfo[1];
  EmitVertex();

  gl_Position =
      GlobalInfo.mvp * TransformData.uMatrix * vec4(vPos[2].xy, 0.0, 1.0);
  fPos = vPos[2];
  fPosInfo = vPosInfo[2];
  EmitVertex();
}
...
void generate_bezier(int quad_in) {
  ...
  for (int i = 0; i < MAX_QUAD_STEP - 1; i++) {
    if (quad_in == 1) {
      gl_Position = GlobalInfo.mvp * TransformData.uMatrix * vec4(p0, 0.0, 1.0);
      fPos = p0;
    } else {
      gl_Position = GlobalInfo.mvp * TransformData.uMatrix * vec4(p1, 0.0, 1.0);
      fPos = p1;
    }
    EmitVertex();
...
void generate_bezier_stroke() {
  ...
  for (int i = 0; i < MAX_QUAD_STEP; i++) {
    ...
    gl_Position = GlobalInfo.mvp * TransformData.uMatrix * vec4(up, 0.0, 1.0);
    fPos = up;
    EmitVertex();

    gl_Position = GlobalInfo.mvp * TransformData.uMatrix * vec4(dp, 0.0, 1.0);
    fPos = dp;
    EmitVertex();
